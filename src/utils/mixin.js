...
function compose(...funcs) {
  ...
  return (...args) => rest.reduceRight((composed, f) => f(...args, composed), last(...args));
}
...
const PAGE_EVENT = ['onLoad', 'onReady', 'onShow', 'onHide', 'onUnload', 'onPullDownRefresh', 'onReachBottom'];
...
const getMixinMethods = mixins => {
  ...
  mixins.forEach(mixin => {
    let { data, onLoad, onReady, onShow, onHide, onUnload, onPullDownRefresh, onReachBottom, ...methods } = mixin;
  ...
  Object.keys(mixinMethods).forEach(key => {
    ...
    if (PAGE_EVENT.includes(key)) {
      ...
      pageConf[key] = (function() {
        return function() {
          compose(...methodsList.map(f => f.bind(this)))(...arguments);
        }
      })()
    }
